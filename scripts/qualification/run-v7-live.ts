...
async function run() {
  ...
  await executeWrite({client, account, state, abi, label: "core:create_intent", address: core, functionName: "create_intent", args: [mandateId, counterpartyId, calldataAddress(CalldataAddress, EXPECTED_SIGNER), AMOUNT, "Exact GenLayer artifact delivery", `Retrieve and deliver ${artifactIdentity}.`, `The deliverable is the complete authenticated artifact ${artifactIdentity}.`, "Exactly 1 smallest native GEN unit (0.000000000000000001 GEN); no commercial expansion or substitution.", "Completion requires the complete committed artifact and exact identity to be authenticated.", BigInt(intentExpiry)], postcondition: async () => asRecord(await read(client, core, "get_intent", [intentId], latestFinal))});
