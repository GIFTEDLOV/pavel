...
async function run() {
  ...
  let validFrom = beforeMandate.chainNow + MANDATE_ACTIVATION_MARGIN;
  let expiresAt = validFrom + MANDATE_HORIZON;
  ...
  const makeMandateArgs = () => [mandateId, "PAVEL V8 challenge and fulfillment qualification", "Authorize bounded retrieval, authentication, fulfillment review, and challenge review for the exact immutable protocol artifact.", "The principal retains control; the registered agent may submit only the bounded intent.", "Retrieve, authenticate, and deliver the exact committed PAVEL protocol artifact identified by URL, SHA-256, and byte length.", "No unrelated activity, recipient change, budget expansion, substitution, or external transfer beyond exact artifact delivery and protocol challenge review.", AMOUNT, AMOUNT, 86400n, AMOUNT, BigInt(validFrom), BigInt(expiresAt), BigInt(CHALLENGE_WINDOW), "Authenticated HTTPS evidence with the committed URL, SHA-256, and byte length is required for authorization and fulfillment; challenge evidence must be independently authenticated.", AUTHORITY, "Fulfillment requires the complete authenticated artifact with its committed identity; a qualifying challenge may block settlement until resolution or expiry.", "Use only the approved raw.githubusercontent.com authority; unresolved fulfillment expires to refund.", false];
  await executeWrite({client, account, state, abi, label: "core:configure_mandate", address: core, functionName: "configure_mandate", args: makeMandateArgs(), postcondition: async () => asRecord(await readLatest(client, core, "get_mandate", [mandateId], latestFinal))});
  let canonicalMandate = asRecord(await readLatest(client, core, "get_mandate", [mandateId], latestFinal));
  const canonicalRemaining = Number(canonicalMandate.valid_from ?? "0") - (await chainTime(client)).chainNow;
  if (canonicalMandate.status === "DRAFT" && canonicalRemaining < 20) {
    if (state.steps["core:configure_mandate:corrected"]?.tx) throw new Error("Corrected Mandate window is also too close; refusing a third timing attempt");
    const refreshed = await chainTime(client);
    validFrom = refreshed.chainNow + MANDATE_ACTIVATION_MARGIN + 60;
    expiresAt = validFrom + MANDATE_HORIZON;
    await executeWrite({client, account, state, abi, label: "core:configure_mandate:corrected", address: core, functionName: "configure_mandate", args: makeMandateArgs(), postcondition: async () => asRecord(await readLatest(client, core, "get_mandate", [mandateId], latestFinal))});
    canonicalMandate = asRecord(await readLatest(client, core, "get_mandate", [mandateId], latestFinal));
  }
  await executeWrite({client, account, state, abi, label: "core:seal_mandate", address: core, functionName: "seal_mandate", args: [mandateId], precondition: async () => { const now = await chainTime(client); const current = asRecord(await readLatest(client, core, "get_mandate", [mandateId], latestFinal)); const remaining = Number(current.valid_from ?? "0") - now.chainNow; if (current.status !== "DRAFT" || remaining < 20) throw new Error(`Mandate activation margin is too small to seal safely: status=${current.status} remaining=${remaining}`); }, postcondition: async () => asRecord(await readLatest(client, core, "get_mandate", [mandateId], latestFinal))});
  ...
  await waitForChainTime(client, Number(sealed.valid_from), "MANDATE_ACTIVATION");
  ...
  const intentExpiry = Math.min(Number(sealed.expires_at), (await chainTime(client)).chainNow + 7200);
  ...
  const stagedFulfillment = await executeWrite({client, account, state, abi, label: "core:stage_evidence:fulfillment", address: core, functionName: "stage_evidence", args: [intentId], postcondition: async () => { const item = asRecord(await readLatest(client, core, "get_intent", [intentId], latestFinal)); const definition = asRecord(await readLatest(client, core, "get_evidence", [intentId, 1n], latestFinal)); if (item.status !== "FULFILLMENT_PENDING" || definition.evidence_kind !== "FULFILLMENT" || definition.sequence !== "1") throw new Error(`Sequence-one fulfillment evidence was not staged canonically: status=${item.status}`); const snapshot = asRecord(await readLatest(client, core, "get_snapshot", [item.current_snapshot_id], latestFinal)); const capture = Array.isArray(snapshot.captures) ? snapshot.captures.find((entry: AnyRecord) => entry.evidence_id === definition.evidence_id) : undefined; if (!capture || capture.capture_class !== "AUTHENTICATED" || capture.sha256 !== definition.committed_sha256 || String(capture.byte_length) !== String(definition.committed_byte_length)) throw new Error("Sequence-one fulfillment capture is not authenticated in the canonical snapshot"); return {intent: item, definition, snapshot}; }});
  ...
  const priorBlockingProof = state.observations.challengeBlockingProof;
  const blockedSettlement = priorBlockingProof?.settlement
    ? asRecord(priorBlockingProof.settlement)
    : asRecord(await readLatest(client, core, "get_settlement_instruction", [intentId], latestFinal));
  if (!priorBlockingProof) {
