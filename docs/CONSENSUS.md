...
The V6 authorization leader returns compact JSON with the exact `pavel-authorization-v2` schema: one schema key and twelve JSON booleans. The validator checks the `gl.vm.Return` wrapper, exact key set, schema version, actual boolean types, and independent state-consequential fields. Missing keys, extra keys, wrong types, malformed JSON, free-form prose, and inconsistent vectors are rejected. No free-form explanation participates in authorization consensus.
