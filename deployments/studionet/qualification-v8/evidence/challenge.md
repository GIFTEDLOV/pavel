...
Every settlement is namespaced by `SETTLE:V1:<hash(intent_id,direction)>`, stored before emission, and rejected if the Intent already has a settlement ID. Blind retries are forbidden. V7 exposes `UNCONFIRMED` pending state and does not pretend to prove child transfer completion. The final qualification state is `RELEASE_PENDING` with conserved accounting and no claim of external payment completion.
